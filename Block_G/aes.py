...
open_text = '00112233445566778899aabbccddeeff'
...
#   Функция разделяет строку на подстроки по 2 символа.
#   Args:
...
# ]  
open_text_mas = split_string_by_2(open_text)
state = [
    [],
    [],
    [],
...
def sub_bytes(statef):
    ...
    for i in range(4):
        for j in range(4):
            byte = state_res[i][j]
            res = s_box[hex.index(byte[0])][hex.index(byte[1])]
            state_res[i][j] = res
    return state_res
